...
import {
  employees, clients, clientUnits, jobFunctions, clientFunctions,
  shifts, costCenters, suppliers, bankAccounts,
} from "../../drizzle/schema";
...
export const cadastrosRouter = router({
  ...
  employees: router({
    list: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        await requirePermission(ctx.user.id, "employees", "canView");
        ...
        if (input?.search) {
          ...
          filtered = filtered.filter(e => e.name.toLowerCase().includes(s) || e.cpf?.includes(s) || e.city?.toLowerCase().includes(s));
        }
        if (input?.status) {
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        await requirePermission(ctx.user.id, "employees", "canCreate");
        const db = await getDb();
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        await requirePermission(ctx.user.id, "employees", "canEdit");
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
        const { id, ...data } = input;
    ...
    delete: protectedProcedure.input(z.number()).mutation(async ({ ctx, input }) => {
      await requirePermission(ctx.user.id, "employees", "canDelete");
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
  ...
  clients: router({
    list: protectedProcedure.input(z.object({ search: z.string().optional() }).optional()).query(async ({ ctx, input }) => {
      await requirePermission(ctx.user.id, "clients", "canView");
      const db = await getDb();
      if (!db) return [];
      const results = await db.select().from(clients).orderBy(desc(clients.createdAt));
      if (input?.search) {
        const s = input.search.toLowerCase();
        return results.filter(c => c.name.toLowerCase().includes(s) || c.cnpj?.includes(s));
      }
      return results;
    }),
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        await requirePermission(ctx.user.id, "clients", "canCreate");
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        await requirePermission(ctx.user.id, "clients", "canEdit");
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
        const { id, ...data } = input;
    ...
    delete: protectedProcedure.input(z.number()).mutation(async ({ ctx, input }) => {
      await requirePermission(ctx.user.id, "clients", "canDelete");
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
  ...
  clientUnits: router({
    listByClient: protectedProcedure.input(z.number()).query(async ({ ctx, input }) => {
      ...
      const db = await getDb();
      if (!db) return [];
      return db.select().from(clientUnits).where(eq(clientUnits.clientId, input));
    }),

    create: protectedProcedure
      .input(z.object({ clientId: z.number(), name: z.string().min(1), address: z.string().optional() }))
      .mutation(async ({ ctx, input }) => {
        await requirePermission(ctx.user.id, "clients", "canEdit");
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
  ...
  jobFunctions: router({
    list: protectedProcedure.query(async ({ ctx }) => {
      await requirePermission(ctx.user.id, "functions", "canView");
      const db = await getDb();
      if (!db) return [];
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        await requirePermission(ctx.user.id, "functions", "canCreate");
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        await requirePermission(ctx.user.id, "functions", "canEdit");
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
        const { id, ...data } = input;
    ...
    delete: protectedProcedure.input(z.number()).mutation(async ({ ctx, input }) => {
      await requirePermission(ctx.user.id, "functions", "canDelete");
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
  ...
  shifts: router({
    list: protectedProcedure.query(async ({ ctx }) => {
      await requirePermission(ctx.user.id, "shifts", "canView");
      const db = await getDb();
      if (!db) return [];
    ...
    create: protectedProcedure
      .input(z.object({ name: z.string().min(1), startTime: z.string(), endTime: z.string() }))
      .mutation(async ({ ctx, input }) => {
        await requirePermission(ctx.user.id, "shifts", "canCreate");
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        await requirePermission(ctx.user.id, "shifts", "canEdit");
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
        const { id, ...data } = input;
    ...
    delete: protectedProcedure.input(z.number()).mutation(async ({ ctx, input }) => {
      await requirePermission(ctx.user.id, "shifts", "canDelete");
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
  ...
  costCenters: router({
    list: protectedProcedure.query(async ({ ctx }) => {
      await requirePermission(ctx.user.id, "cost_centers", "canView");
      const db = await getDb();
      if (!db) return [];
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        await requirePermission(ctx.user.id, "cost_centers", "canCreate");
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    ...
    delete: protectedProcedure.input(z.number()).mutation(async ({ ctx, input }) => {
      await requirePermission(ctx.user.id, "cost_centers", "canDelete");
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
  ...
  suppliers: router({
    list: protectedProcedure.input(z.object({ search: z.string().optional() }).optional()).query(async ({ ctx, input }) => {
      await requirePermission(ctx.user.id, "suppliers", "canView");
      const db = await getDb();
      if (!db) return [];
      const results = await db.select().from(suppliers).orderBy(suppliers.name);
      if (input?.search) {
        const s = input.search.toLowerCase();
        return results.filter(s2 => s2.name.toLowerCase().includes(s) || s2.cnpj?.includes(s));
      }
      return results;
    }),
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        await requirePermission(ctx.user.id, "suppliers", "canCreate");
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        await requirePermission(ctx.user.id, "suppliers", "canEdit");
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
        const { id, ...data } = input;
    ...
    delete: protectedProcedure.input(z.number()).mutation(async ({ ctx, input }) => {
      await requirePermission(ctx.user.id, "suppliers", "canDelete");
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
  ...
  bankAccounts: router({
    list: protectedProcedure.query(async ({ ctx }) => {
      await requirePermission(ctx.user.id, "bank_accounts", "canView");
      const db = await getDb();
      if (!db) return [];
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        await requirePermission(ctx.user.id, "bank_accounts", "canCreate");
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        await requirePermission(ctx.user.id, "bank_accounts", "canEdit");
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
        const { id, ...data } = input;
    ...
    delete: protectedProcedure.input(z.number()).mutation(async ({ ctx, input }) => {
      await requirePermission(ctx.user.id, "bank_accounts", "canDelete");
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
